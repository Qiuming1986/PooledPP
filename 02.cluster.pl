...
$max_distance_CGR            = 360;  # Example default for CGR
$max_distance_duplication    = 220;  # Example default for Duplication
$max_distance_SCGR           = 410;  # Example default for SCGR
...
if ($intermediate_output_file){
    # 打开中间结果文件
    open (MEDOUT,">$intermediate_output_file");
    print MEDOUT "Chromosome\tanchor_Ref_Start\tanchor_Ref_End\tRef_Start\tRef_End\tRead_Name\tSV_Type\tSupport\tStrand\tRef_Path\tRead_Path\tanchor_Sequence\n";
}
...
# sort input file
my $tmp_file = $output_file.".tmp.txt";
system("csvtk sort -t -k 1 -k 4:n -k 5:n $input_file > $tmp_file");

open my $in_fh, '<', $tmp_file or die $!;


my ($prev_chr, $prev_refstart, $prev_refend) = ('', 0, 0);
my @block_svs;
...
while (<$in_fh>) {
    ...
    next if /^\s*$/;
    my @f = split /\t/;
    my ($chr, $anchor_ref_start, $anchor_ref_end, $ref_start, $ref_end) =
        @f[0,1,2,3,4];
    # detect block boundary
    if ($prev_chr ne '' 
        && ($chr ne $prev_chr
            || $ref_start - $prev_refstart > $block_gap)) {
        process_block(\@block_svs);
        @block_svs = ();
    }
    # 复制其余字段到 hash
    my %sv = (
    ...
    ($prev_chr, $prev_refstart, $prev_refend) =
      ($chr, $ref_start, $ref_end);
}
# 最后 flush
process_block(\@block_svs) if @block_svs;

close $in_fh;

# process_block: 集中执行你原有的第一、二、三阶段 clustering 逻辑
sub process_block {
    my ($svs_ref) = @_;
    my @sv_list = @{$svs_ref};
    return unless @sv_list;
    ...
    foreach my $sv (@sv_list) {

        # Unpack fields into variables
        my $chromosome = $sv->{chromosome};
        my $anchor_ref_start = $sv->{anchor_ref_start};
        my $anchor_ref_end = $sv->{anchor_ref_end};
        my $ref_start = $sv->{ref_start};
        my $ref_end = $sv->{ref_end};
        my $read_name = $sv->{5}; 
        my $anchor_read_start = $sv->{6};
        my $anchor_read_end = $sv->{7};
        my $read_start = $sv->{8};
        my $read_end = $sv->{9};
         my $sv_type = $sv->{10};
        my $strand = $sv->{11};
        my $integrity = $sv->{12};
        my $left_anchor_score = $sv->{13};
        my $right_anchor_score = $sv->{14};
        my $ref_path = $sv->{15};
        my $read_path = $sv->{16};
        my $origin_seq = $sv->{17};
        my $anchor_seq = $sv->{18};
        ...
        if ($sv_type eq "Duplication") {
            ...
            while ($continue_merging) {
                ...
                if ($control > 1) {
                    ...
                    if (exists $node_bounds_Dup{$chromosome}{$ref_start}{$ref_end}{$curr_sv->{read_name}}) {
                        @min_max_merge_bounds = @{$node_bounds_Dup{$chromosome}{$ref_start}{$ref_end}{$curr_sv->{read_name}}};
                ...
                if (exists $node_bounds_Dup{$chromosome}{$ref_start}{$ref_end}) {
                    #my $bound_count = scalar keys %{$node_bounds_Dup{$chromosome}{$ref_start}{$ref_end}};
                    #print "Number of node bounds for $chromosome:$ref_start-$ref_end: $bound_count\n";
                    foreach my $existing_read_name (keys %{$node_bounds_Dup{$chromosome}{$ref_start}{$ref_end}}) {
                        ...
                        my @min_max_target_bounds = @{$node_bounds_Dup{$chromosome}{$ref_start}{$ref_end}{$existing_read_name}};
                        ...
                        if ($merge) {
                            ...
                            # 更新节点边界
                            update_node_bounds_Dup(\@min_max_merge_bounds, \@min_max_target_bounds);
                            ...
                            if ($existing_sv->{integrity} eq "leftSpanRightSpan" && $curr_sv->{integrity} eq "leftSpanRightSpan") {
                                ...
                                if ($curr_score > $existing_score) {
                                    ...
                                    # 更新 node_bounds_Dup，删除旧的 existing_read_name
                                    #delete $node_bounds_Dup{$chromosome}{$ref_start}{$ref_end}{$existing_read_name};        
                                    $node_bounds_Dup{$chromosome}{$ref_start}{$ref_end}{$curr_sv->{read_name}} = \@min_max_merge_bounds;
                                } else {
                                    $existing_sv->{support} += $curr_sv->{support};
                                    delete $seen_sv{$sv_key};
                                    $curr_sv = $existing_sv; # 将 curr_sv 更新为 existing_sv
                                    #delete $node_bounds_Dup{$chromosome}{$ref_start}{$ref_end}{$curr_sv->{read_name}};  
                                    $node_bounds_Dup{$chromosome}{$ref_start}{$ref_end}{$existing_read_name} = \@min_max_merge_bounds;
                                }
            ...
            if (!$merged) {
                $node_bounds_Dup{$chromosome}{$ref_start}{$ref_end}{$read_name} = \@min_max_merge_bounds;
                $seen_sv{$sv_key} = $curr_sv;
        ...
        # 处理 SCGR 类型的 SV 合并
            ...
            while ($continue_merging) {
                ...
                if ($control > 1) {
                    # For subsequent merges, use the existing node bounds from curr_sv
                    if (exists $node_bounds_SCGR{$chromosome}{$ref_start}{$ref_end}{$curr_sv->{read_name}}{$curr_sv->{ref_path}}) {
                        @min_max_merge_bounds = @{$node_bounds_SCGR{$chromosome}{$ref_start}{$ref_end}{$curr_sv->{read_name}}{$curr_sv->{ref_path}}};
                ...
                # Check node bounds and perform merging if possible
                if (exists $node_bounds_SCGR{$chromosome}{$ref_start}{$ref_end}){
                    foreach my $existing_read_name (keys %{$node_bounds_SCGR{$chromosome}{$ref_start}{$ref_end}}) {
                        foreach my $existing_ref_path (keys %{$node_bounds_SCGR{$chromosome}{$ref_start}{$ref_end}{$existing_read_name}}){
             
                            my $existing_sv_key = join("_", $chromosome, $ref_start, $ref_end, $existing_read_name, $existing_ref_path);
                            my $existing_sv = $seen_sv{$existing_sv_key};  
                            next unless (exists $seen_sv{$existing_sv_key});
                            next if ($existing_sv_key eq $sv_key);
                            my @min_max_target_bounds = @{$node_bounds_SCGR{$chromosome}{$ref_start}{$ref_end}{$existing_read_name}{$existing_ref_path}};
                            ...
                            if ($merge) {
                                ...
                                if ($existing_sv->{integrity} eq "leftSpanRightSpan" && $curr_sv->{integrity} eq "leftSpanRightSpan") {
                                    ...
                                    if ($curr_score > $existing_score) {
                                        $curr_sv->{support} += $existing_sv->{support};
                                        delete $seen_sv{$existing_sv_key};
                                        # 更新 node_bounds_SCGR，删除旧的 existing_read_name
                                        #delete $node_bounds_SCGR{$chromosome}{$ref_start}{$ref_end}{$existing_read_name};        
                                        $node_bounds_SCGR{$chromosome}{$ref_start}{$ref_end}{$curr_sv->{read_name}}{$curr_sv->{ref_path}} = \@min_max_merge_bounds;
                                    } else {
                                        $existing_sv->{support} += $curr_sv->{support};
                                        delete $seen_sv{$sv_key};
                                        $curr_sv = $existing_sv; # 将 curr_sv 更新为 existing_sv
                                        #delete $node_bounds_SCGR{$chromosome}{$ref_start}{$ref_end}{$curr_sv->{read_name}};  
                                        $node_bounds_SCGR{$chromosome}{$ref_start}{$ref_end}{$existing_read_name}{$existing_ref_path} = \@min_max_merge_bounds;
                                    }
                                } elsif ($existing_sv->{integrity} eq "leftSpanRightSpan" || $curr_sv->{integrity} eq "leftSpanRightSpan") {
                                    # Case 2: One has "leftSpanRightSpan"
                                    if ($curr_sv->{integrity} eq "leftSpanRightSpan") {
            ...
            if (!$merged) {
                $node_bounds_SCGR{$chromosome}{$ref_start}{$ref_end}{$read_name}{$ref_path} = \@min_max_merge_bounds;
                $seen_sv{$sv_key} = $curr_sv;
                #print "$seen_sv{$sv_key}\n";
                                                    ...
                                                    if ($sv_type_name eq "Duplication") {
                                                        ...
                                                        while ($continue_merging) {
                                                            ...
                                                            foreach my $sv (@final_cluster) {
                                                                ...
                                                                if (exists $node_bounds_Dup{$sv->{chromosome}}{$sv->{ref_start}}{$sv->{ref_end}}{$sv->{read_name}}) {                        
                                                                    my @min_max_target_bounds = @{$node_bounds_Dup{$sv->{chromosome}}{$sv->{ref_start}}{$sv->{ref_end}}{$sv->{read_name}}};
    
                                                                    # Ensure node counts are the same for merging
                                                                    if (exists $node_bounds_Dup{$peak_sv->{chromosome}}{$peak_sv->{ref_start}}{$peak_sv->{ref_end}}{$peak_sv->{read_name}}) {
                                                                        my @min_max_merge_bounds = @{$node_bounds_Dup{$peak_sv->{chromosome}}{$peak_sv->{ref_start}}{$peak_sv->{ref_end}}{$peak_sv->{read_name}}};
                                                                        ...
                                                                        if ($merge) {
                                                                            ...
                                                                            update_node_bounds_Dup(\@min_max_merge_bounds, \@min_max_target_bounds);
                                                                            @{$node_bounds_Dup{$peak_sv->{chromosome}}{$peak_sv->{ref_start}}{$peak_sv->{ref_end}}{$peak_sv->{read_name}}} = \@min_max_merge_bounds;
                                                    ...
                                                    if ($sv_type_name eq "SCGR") {
                                                        ...
                                                        while ($continue_merging) {
                                                            ...
                                                            foreach my $sv (@final_cluster) {
                                                                ...
                                                                # Check if the SV can be merged with peak_sv based on node bounds
                                                                if (exists $node_bounds_SCGR{$sv->{chromosome}}{$sv->{ref_start}}{$sv->{ref_end}}{$sv->{read_name}}{$sv->{ref_path}}) {                        
                                                                    my @min_max_target_bounds = @{$node_bounds_SCGR{$sv->{chromosome}}{$sv->{ref_start}}{$sv->{ref_end}}{$sv->{read_name}}{$sv->{ref_path}}};
    
                                                                    # Ensure node counts are the same for merging
                                                                    if (exists $node_bounds_SCGR{$peak_sv->{chromosome}}{$peak_sv->{ref_start}}{$peak_sv->{ref_end}}{$peak_sv->{read_name}}{$peak_sv->{ref_path}}) {
                                                                        my @min_max_merge_bounds = @{$node_bounds_SCGR{$peak_sv->{chromosome}}{$peak_sv->{ref_start}}{$peak_sv->{ref_end}}{$peak_sv->{read_name}}{$peak_sv->{ref_path}}};
                                                                        ...
                                                                        if ($merge) {
                                                                            # Update bounds after merging
                                                                            update_node_SCGR_bounds(\@min_max_merge_bounds, \@min_max_target_bounds);
                                                                            @{$node_bounds_SCGR{$peak_sv->{chromosome}}{$peak_sv->{ref_start}}{$peak_sv->{ref_end}}{$peak_sv->{read_name}}{$peak_sv->{ref_path}}} = \@min_max_merge_bounds;
                                                                            #print "Updated min_max_merge_bounds: " . Dumper(\@min_max_merge_bounds);
...
if ($intermediate_output_file){
    system("gzip $intermediate_output_file");
	close MEDOUT;
}
...
Options:
    ...
    --max-distance-CGR <int>                 Maximum distance for clustering CGR (complex genomic rearrangement) SVs (default: 360).
    --max-distance-duplication <int>         Maximum distance for clustering Duplication SVs (default: 220).
    --max-distance-SCGR <int>                Maximum distance for clustering SCGR SVs (default: 410).
