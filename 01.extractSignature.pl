...
my ($bam_file, $output_file, $min_mapq, $min_align_len, $min_sv_len, $max_sv_len, $mitochondria_chr,
...
if (!$bam_file || !$output_file) {
    usage();
    exit;
}

# Open the input BAM file using samtools
open(IN, "-|", "samtools view $bam_file") or die "Error: Unable to open BAM file: $bam_file\n";

# Initialize a hash to store alignments grouped by read name
my %read_alignments;

# Array to store output lines, which will be sorted later
my @output_lines;

my $step_size = 100;  # Step size is half of the window size

# Declare global variables to store repeat regions and depth information
my %repeat_regions;
my %depth_data;
my $total_depth = 0;
my $position_count = 0;
...
# Combine %repeat_regions and %graded_intervals for overlap removal
...
# Read the BAM file line by line, parsing each alignment record
while (<IN>) {
    chomp;
    my @fields = split /\t/, $_;
    ...
    my $flag = $fields[1];
    my $ref_pos = $fields[3];  # 0-based start position on the reference
    ...
    # Calculate start and end positions for both read and reference
    my ($read_start, $read_end, $ref_start, $ref_end) = calculate_read_positions($cigar_string, $sequence, $ref_pos);

    # Store the parsed alignment information in a hash, grouping by read name
    push @{$read_alignments{$read_name}}, {
        cigar => $cigar_string,
        flag => $flag,
        ref_pos => $ref_pos,         # Start position on the reference sequence
        ref_chr => $ref_chr,         # Chromosome or contig
        ref_start => $ref_start,     # Calculated reference start position
        ref_end => $ref_end,         # Calculated reference end position
        mapq => $fields[4],          # Mapping quality score
        strand => (($flag & 0x10) ? "-" : "+"),  # Strand (+/-) based on the flag
        seq => $sequence,            # Aligned sequence
        read_start => $read_start,   # Read start position
        read_end => $read_end,       # Read end position
        read_name => $read_name      # Explicitly store the read name
    };
}

print "Bam file processed complete!\n";

# Get the total number of reads after filtering by mapping quality, alignment length, and mitochondrial filtering
my $total_reads = scalar(keys %read_alignments);  
my $processed_reads = 0;

# Progress tracking and status printing
foreach my $read_name (keys %read_alignments) {
    $processed_reads++;

    # Print progress for every 1000 reads processed
    if ($processed_reads % 1000 == 0) {
        print "Processed $processed_reads / $total_reads reads\n";
    }
    
    # Check for reads mapped to multiple chromosomes
    my $alignments = $read_alignments{$read_name};
    my %chromosomes_seen;

    # Track how many different chromosomes the read is mapped to
    foreach my $alignment (@$alignments) {
        $chromosomes_seen{$alignment->{ref_chr}} = 1;
    }
    
    # Track the number of split segments in the read
    my $num_segments = scalar(@$alignments);

    # Skip reads with too many split segments
    my $read_length = length($alignments->[0]->{seq});
    if ($num_segments > 3 + 0.1 * ($read_length / 1000)) {
        next;
    }
    
    #order by reads position
    my @sorted_alignments = sort { $a->{read_start} <=> $b->{read_start} } @$alignments;
    ...
    if (@sorted_alignments >= 2){
        ...
        for (my $i = 0; $i <= @sorted_alignments - 1; $i++) {
            ...
            push @output_lines, $output if $output;
        }
    ...
    if (@sorted_alignments == 1){
        my $type = "simple";
        my $output = process_indels($alignments->[0], $read_name, $type);
        push @output_lines, $output if $output;
    }
...
# Sort the output lines based on chromosome and reference start position
@output_lines = sort {
    my ($a_chr, $a_start) = (split /\t/, $a)[0,1];  # Extract chromosome and reference start for line A
    my ($b_chr, $b_start) = (split /\t/, $b)[0,1];  # Extract chromosome and reference start for line B
    ...
    # If chromosomes are the same, compare by start position
...
# Print each sorted output line to the gzipped file
foreach my $line (@output_lines) {
    print $gz "$line\n";
}

# Close the gzipped output file
$gz->close(); 
...
sub usage {
    print <<END;
Usage: perl $0 --bam <sorted BAM file> --output <SV signature table> [options]
...
Examples:
    perl $0 --bam sample.bam --output sv_signature_table --min-mapq 20 --min-sv-len 50 --max-sv-len 10000
